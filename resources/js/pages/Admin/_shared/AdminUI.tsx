// resources/js/pages/Admin/_shared/AdminUI.tsx
import { Link } from '@inertiajs/react';
import { ChevronLeft, ChevronRight, RefreshCw } from 'lucide-react';
import Swal from 'sweetalert2';
import type { PaginationData, UserSummary } from '../../../services';
import { getDisplayName, getInitials } from '../../Shared/helpers';

export const C = {
    rose: '#1B5C87', pink: '#4FA8D8', bg: '#F9FAFB',
...
export function PageHeader({
    ...
    return (
        <div className="relative overflow-hidden" style={{ background: `linear-gradient(135deg, ${C.rose} 0%, ${C.pink} 100%)` }}>
                ...
                <div className="flex items-center justify-between gap-3 flex-wrap">
                    <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/70">NE Veterinary Clinic · Admin</p>
...
export function StatCard({ icon: Icon, label, value, color = C.rose }: {
...
// Backend sends color *names* (e.g. "green", "red") for several status_color
// fields; map those to hex so StatusPill can use them directly. Anything
// already hex-like passes through unchanged.
...
export function Avatar({ user, size = 'md' }: { user?: UserSummary | null; size?: 'sm' | 'md' | 'lg' }) {
    ...
    return (
        <div className={`${cls} text-white`} style={{ background: `linear-gradient(135deg, ${C.rose} 0%, ${C.pink} 100%)` }}>
            {getInitials(user)}
        </div>
    );
}
...
export async function confirmAction(opts: {
    ...
    const res = await Swal.fire({
        ...
        confirmButtonColor: opts.danger ? C.red : C.rose,
