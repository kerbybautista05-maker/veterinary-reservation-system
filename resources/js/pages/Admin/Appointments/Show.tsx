// resources/js/pages/Admin/Appointments/Show.tsx
import { Head } from '@inertiajs/react';
import { useEffect, useState, useCallback } from 'react';
import { CalendarClock, PawPrint, User as UserIcon, Stethoscope, Check, XCircle, RotateCcw, CheckCircle2 } from 'lucide-react';
import AppLayout from '@/layouts/app-layout';
import { appointmentService } from '../../../services';
import type { Appointment } from '../../../services';
import { PageHeader, BackLink, StatusPill, confirmAction, toastSuccess, toastError, C } from '../_shared/AdminUI';
import { formatPHDateTime } from '../../Shared/helpers';

export default function AppointmentShow({ appointmentId }: { appointmentId: number | string }) {
        ...
        <AppLayout>
            ...
            <div className="min-h-screen" style={{ background: C.bg }}>
                ...
                <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6 space-y-4">
                    <BackLink href="/admin/appointments" label="Back to Appointments" />
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                        ...
                        <div className="grid sm:grid-cols-2 gap-4 mt-5 text-sm">
                            <div className="flex items-center gap-2 text-gray-600"><PawPrint className="w-4 h-4 text-gray-400" />{appt.pet?.name ?? 'Pet'} ({appt.pet?.species})</div>
                            ...
                            <div className="flex items-center gap-2 text-gray-600"><Stethoscope className="w-4 h-4 text-gray-400" />{appt.veterinarian ? `Dr. ${appt.veterinarian.full_name ?? appt.veterinarian.name}` : 'Unassigned'}</div>
