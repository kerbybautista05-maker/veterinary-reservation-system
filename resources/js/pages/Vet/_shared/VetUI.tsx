// resources/js/pages/Vet/_shared/VetUI.tsx
...
export const C = {
    blue: '#1D6FA5', navy: '#0B2545', sky: '#4FA8DA', bg: '#F5F8FB',
...
export function PageHeader({
    ...
    return (
        <div className="relative overflow-hidden" style={{ background: `linear-gradient(135deg, ${C.blue} 0%, ${C.sky} 100%)` }}>
...
export function StatCard({ icon: Icon, label, value, color = C.blue }: {
...
export function Avatar({ user, size = 'md' }: { user?: UserSummary | null; size?: 'sm' | 'md' | 'lg' }) {
    ...
    return (
        <div className={`${cls} text-white`} style={{ background: `linear-gradient(135deg, ${C.blue} 0%, ${C.sky} 100%)` }}>
            {getInitials(user)}
        </div>
    );
}
...
export function PetAvatar({ photoUrl, size = 'md' }: { photoUrl?: string | null; size?: 'sm' | 'md' | 'lg' }) {
    ...
    return (
        <div className={`${sizeMap[size]} rounded-xl flex items-center justify-center shrink-0`} style={{ background: `${C.blue}15` }}>
...
export async function confirmAction(opts: {
    ...
    const res = await Swal.fire({
        ...
        confirmButtonColor: opts.danger ? C.red : C.blue,
