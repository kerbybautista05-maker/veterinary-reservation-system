// resources/js/pages/Vet/Notifications.tsx
...
import { PageHeader, EmptyState, Pagination, toastSuccess, C } from '@/pages/Vet/_shared/VetUI';
import { timeAgo } from '@/pages/Shared/helpers';
                            ...
                            <div className="divide-y divide-gray-50">
                                {items.map(n => (
                                    <div key={n.id} onClick={() => handleOpen(n)}
                                        className={`flex items-start gap-3 px-5 py-3.5 hover:bg-gray-50 transition-colors cursor-pointer ${!n.is_read ? 'bg-blue-50/40' : ''}`}>
                                        {!n.is_read && <span className="w-2 h-2 rounded-full mt-1.5 shrink-0" style={{ background: C.blue }} />}
                                        ...
                                        {n.deep_link && (
                                            <Link href={n.deep_link} className="text-xs font-bold shrink-0" style={{ color: C.blue }}>View</Link>
                                        )}
                                    </div>
                                ))}
                            </div>
