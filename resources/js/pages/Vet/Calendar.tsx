// resources/js/pages/Vet/Calendar.tsx
...
import { PageHeader, C } from '@/pages/Vet/_shared/VetUI';
    ...
    return (
        <AppLayout>
            ...
            <div className="min-h-screen" style={{ background: C.bg }}>
                <PageHeader icon={CalendarDays} title="My Calendar" subtitle="Your appointment schedule" onRefresh={fetchData} />
                            ...
                            {weeks.flat().map((d, i) => {
                                ...
                                return (
                                    <div key={i} className={`min-h-[100px] border-b border-r border-gray-50 p-1.5 ${isCurrentMonth ? '' : 'bg-gray-50/50'}`}>
                                        ...
                                        }`} style={isToday ? { background: C.blue } : undefined}>
                                            {d.getDate()}
                                        </p>
                                        <div className="space-y-0.5">
                                            {dayAppts.slice(0, 2).map(a => (
                                                <Link key={a.id} href={`/vet/appointments/${a.id}`}
                                                    className={`block text-[10px] font-semibold px-1.5 py-0.5 rounded truncate ${a.type === 'emergency' ? 'bg-red-100 text-red-700' : 'bg-blue-50 text-blue-700'}`}>
